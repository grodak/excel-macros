...
For i = 2 To ws.Cells(Rows.Count, 12).End(xlUp).Row
    ...
    ws2.ExportAsFixedFormat Type:=xlTypePDF, fileName:="x:\xxx\xxx\" & fileName & "ChangeReport" & Format(Now - 7, "yyyymmdd") & "-" & Format(Now, "yyyymmdd"), Quality:=xlQualityStandard, IncludeDocProperties:=True, IgnorePrintAreas:=False, OpenAfterPublish:=False
Next
